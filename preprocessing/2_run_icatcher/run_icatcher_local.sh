...
if [[ "$face_classifier" = "baby" ]] ; then
    cmd="icatcher $current_video --use_fc_model --illegal_transitions_path $icatcher_scripts_path/illegal_transitions.csv --output_annotation $full_output_annotations_path --output_format compressed --output_video_path $full_output_videos_path --crop_percent $cropamt --crop_mode $cropside --gpu_id $gpu_id"
elif [[ "$face_classifier" = "lowest" ]] ; then
    cmd="icatcher $current_video --output_annotation $full_output_annotations_path --output_format compressed --output_video_path $full_output_videos_path --crop_percent $cropamt --crop_mode $cropside --gpu_id $gpu_id"
fi
echo $cmd 
