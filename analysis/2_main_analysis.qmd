---
title: "Main visual precision descriptive analyses"
format:
  html:
    toc: true
---
...
# sample1/sample2 live together in data/main/ with a 'section' column in trial_metadata.
...
# TODO: move similarity correlations to single file
cvcl_similarities <- read.csv(here("data", "embeddings", "similarities-cvcl_data.csv"))
clip_similarities <- read.csv(here("data", "embeddings", "similarities-clip_data.csv"))
...
create_model_plots(bv_dino_similarities, name="BV-DINO", median_age=input_median_age)
create_model_plots(clip_similarities, name="CLIP", median_age=input_median_age)
...
cor.test((imagenetvit_similarities |> arrange(word1, word2))$image_similarity, (trial_metadata |> distinct(Trials.imagePair, .keep_all=TRUE) |>  filter(section=="sample1") |> arrange(Trials.imagePair))$image_similarity, method="spearman")
...
# layer-wise
```{r}
layerwise <- read.csv(here("data/embeddings/layerwise_similarities.csv"))
...
layerwise_combined <- layerwise_long |>
  bind_rows(
    layerwise_long |>
      rename(text1 = text2, text2 = text1)
  )

looking_data_w_layerwise <- looking_data_summarized |>
  select(-image_similarity) |>
  left_join(layerwise_combined, by = c("Trials.distractorImage" = "text1", "Trials.targetImage" = "text2"), relationship="many-to-many")
...
ggplot(image_correlation_results_layerwise, aes(x = layer, y = pearson_cor)) +
  ...
  labs(
    title = "Image similarity correlation across ViT layers",
    x = "Layer",
    y = "Pearson Correlation"
  ) +
  theme_minimal() +
...
layerwise_models <- layerwise_combined |>
  group_by(layer) |>
  group_split() |>
  map(function(layer_data) {
    layer_num <- unique(layer_data$layer)
    
    d <- trials_with_effect_vars |>
  select(-any_of("image_similarity")) |>
  left_join(layer_data, by = c("Trials.distractorImage" = "text1", "Trials.targetImage" = "text2"),
