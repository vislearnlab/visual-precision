...
looking_times_target <- ggplot(summarize_target, 
                              aes(time_normalized_corrected, 
                                  mean_accuracy, 
                                  color = Trials.targetImage,  # Use target image for color mapping
                                  group = Trials.targetImage)) +  # Group by target image
  xlim(-2000, 4000) +
  geom_errorbar(aes(ymin = mean_accuracy - ci, 
                    ymax = mean_accuracy + ci), 
                width = 0, 
                alpha = 0.15) +
  ...
  ylab("Proportion Target Looking") +
   scale_color_manual(values = target_colors,
                     name = "Primary Target Words \n(sorted in increasing\nage-of-acquisition\norder)") +
  theme_minimal()

# Display the plot
looking_times_target
ggsave(here("figures",PROJECT_VERSION,"timecourse_plot.svg"),looking_times_target,width=8.5,height=5.5,bg = "white")
```
...
participant_age_halves = summarized_data(looking_data_summarized |> left_join(target_looking_item_level) |> rename(mean_looking = mean_value) |>  add_age_split() |> mutate(acc = corrected_target_looking_short), "age_half", "acc", 
...
all_clip_plot
ggsave(here("figures",PROJECT_VERSION,"clip_plot.svg"),all_clip_plot,width=8.5, height=8,bg = "white")
```
...
aoa_across_time <- ggplot(aoa_jittered, aes(x=aoa_x_jittered, y=mean_value))+
  geom_hline(yintercept=0,linetype="dashed")+
  geom_point(size=6, alpha=0.4, color="#08306B")+
  geom_linerange(aes(ymin = mean_value - ci, ymax = mean_value + ci), alpha = 0.1) + 
  geom_smooth(method="lm", color="#08306B", alpha=0.5)+
  geom_label_repel(aes(label=ifelse((Trials.targetImage %in% c("swan", "acorn", "key", "worm", "potato", "goat", "bulldozer")), Trials.targetImage, ""), force_pull=0.2)) +
  xlab("Estimated age-of-acquisition of target words")+
...
aoa_across_time
ggsave(here("figures",PROJECT_VERSION,"aoa_plot.svg"),aoa_across_time,width=8.5,height=5.5,bg = "white")
