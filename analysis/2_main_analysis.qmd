...
#TODO: we have a participant who participated twice but their first session only had 2 trials. figure out how to deal with this/discard first session trials
```
...
```{r}
participant_age_halves = summarized_data(looking_data_summarized |> left_join(target_looking_item_level) |> rename(mean_looking = mean_value) |>  add_age_split() |> mutate(acc = corrected_target_looking + 0.5), "age_half", "acc", 
...
```{r}
# filter(AoA_Est_target < 4.5 & SubjectInfo.testAge < 19.8*30)
...
set.seed(2)
avg_corrected_target_looking_by_condition <- avg_corrected_target_looking_by_condition |> filter(Trials.trialType == "easy" | Trials.trialType == "hard") #filter(SubjectInfo.subjID %in% high_accuracy_subjects$SubjectInfo.subjID)
...
calculate_correlations <- function(data, x_var, y_var, group_var=c("epoch")) {
  data |>
    ...
    summarize(pearson_cor = cor(.data[[x_var]], .data[[y_var]], method = "spearman"),
          p_value = cor.test(.data[[x_var]], .data[[y_var]], method = "spearman")$p.value,
        .groups = "drop")
}
...
# AoA plots
```{r}
ggplot(target_looking_item_age_level, aes(x=AoA_Est_target, y=mean_value, color=age_half))+
  geom_point()+
  ...
  theme(axis.title.x = element_text(face="bold", size=15, vjust=-1),
     ...
     labs(caption = (paste0("14-24 month olds age split at M=",mean_age," months"))) +
