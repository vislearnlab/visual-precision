...
looking_time_resampled_clean <- read.csv(file.path(PROCESSED_DATA_PATH, "looking_times_resampled_clean.csv"))
all_looking_times <- read.csv(file.path(PROCESSED_DATA_PATH,"looking_times_with_exclusion_info.csv"))
trial_metadata <- read.csv(here("data","metadata","trial_metadata.csv"))
trial_summary_data <- read.csv(file.path(PROCESSED_DATA_PATH,"trial_summary_data.csv"))
```
...
looking_times
ggsave(here("figures",PROJECT_VERSION,"prop_looking_across_time_and_subject.png"),looking_times,width=9,height=6,bg = "white")
```
...
subj_performances
ggsave(here("figures",PROJECT_VERSION,"subj_performances.png"),subj_performances,width=9,height=7,bg = "white")
```
...
ggplot(summarized_data(looking_data_summarized, "Trials.trialID", "corrected_target_looking", c("Trials.trialID", "Trials.targetImage", "Trials.distractorImage", "text_similarity", "image_similarity", "cor")), aes(x=text_similarity, y = mean_value)) +
  geom_point() +
