...
### Load data
```{r}
PROCESSED_DATA_SAMPLE1 = here("data","sample1","processed_data")
PROCESSED_DATA_SAMPLE2 = here("data","sample2","processed_data")
looking_time_resampled_clean <- rbind(read.csv(file.path(PROCESSED_DATA_SAMPLE1, "level-looks_data.csv")), read.csv(file.path(PROCESSED_DATA_SAMPLE2, "level-looks_data.csv")))


trial_metadata <- rbind(read.csv(here("data","metadata","level-trialtype_data.csv")) |> mutate(section="sample1"), read.csv(here("data", "metadata", PROJECT_VERSION, "level-trialtype_data.csv")) |> mutate(section="sample2"))
raw_metadata <- read.csv(here("stimuli/lookit/preprocessing/data/level-imagepair_raw_data.csv")) |>
...
trial_metadata <- trial_metadata |> select(-image_similarity, -text_similarity) |> 
  left_join(raw_metadata |> transmute(Trials.imagePair = paste0(text1, "-", text2), image_similarity, text_similarity))
trial_summary_data <- rbind(read.csv(file.path(PROCESSED_DATA_SAMPLE1,"level-trials_data.csv")), read.csv(file.path(PROCESSED_DATA_SAMPLE2, "level-trials_data.csv")))
