...
age_excluded_infants <- summarize_subj_trials |>
  filter(age_exclusion == 1 & !(SubjectInfo.subjID %in% participant_total_usable_trials$SubjectInfo.subjID)) |>
  distinct(SubjectInfo.subjID, SubjectInfo.testAge)
...
print(paste("Excluded participants:", sum(participant_total_usable_trials$exclude_participant_insufficient_data,na.rm=T) + num_age_excluded))
...
all_looking_times <- all_looking_times %>%
  mutate(
    ...
    # Infants excluded because they completed all of their trials out of the required age range
