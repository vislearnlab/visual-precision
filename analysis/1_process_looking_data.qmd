...
write_section_csv <- function(df, path) {
  if (!is.null(CURRENT_SECTION) && file.exists(path)) {
    existing <- read_csv(path, show_col_types = FALSE)
    existing <- existing |> filter(section != CURRENT_SECTION)
...
fix_date <- function(x) {
  ...
  # Bogus year (≤ 2010) with slashes: remap month from year digits
  needs_fix <- str_detect(x, "^\\d{4}/") & as.integer(str_extract(x, "^\\d{4}")) <= 2010
  fix_idx <- which(needs_fix)
  if (length(fix_idx) > 0) {
    parts <- str_split_fixed(x[fix_idx], "/", 3)
    d[fix_idx] <- mdy(paste0(str_sub(parts[,1], 3, 4), "/", parts[,2], "/", parts[,3]))
  }
  
  # Valid YYYY/M/D with slashes (year > 2010)
  ymd_slash_idx <- which(str_detect(x, "^\\d{4}/") & !needs_fix)
  if (length(ymd_slash_idx) > 0) d[ymd_slash_idx] <- ymd(x[ymd_slash_idx])
  
  # ISO YYYY-MM-DD with dashes
  ymd_dash_idx <- which(str_detect(x, "^\\d{4}-"))
  if (length(ymd_dash_idx) > 0) d[ymd_dash_idx] <- ymd(x[ymd_dash_idx])
  
  # Standard M/D/YY or M/D/YYYY
  mdy_idx <- which(!str_detect(x, "^\\d{4}"))
  if (length(mdy_idx) > 0) d[mdy_idx] <- mdy(x[mdy_idx])
...
trial_metadata <- trial_metadata |>
  left_join(similarities, by = c("Trials.distractorImage"="word2", "Trials.targetImage"="word1")) |>
...
write_section_csv(trial_metadata, trialtype_path)
```
